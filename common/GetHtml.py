...
class GetHtml:
    ...
    def set(self, url, header=None, retryTimes=10, postData=False):
        '''
        url: url to get
        ...
        if(postData):
            self.postData = urllib.parse.urlencode(postData).encode()
        else:
            self.postData = None

    def get(self, getcookie=False, t_out=20, use_proxy=False, wait_time=1):
        req = urllib.request.Request(self._url, data=self.postData)
        if(self._header):
                ...
                if(not use_proxy):
                    ...
                    for i in range(len(self.proxy)):
                        try:
                            ...
                            req.set_proxy(host=self.proxy[p_i], type="https")
                            req.set_proxy(host=self.proxy[p_i], type="https")
